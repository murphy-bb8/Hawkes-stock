"""
4D Toxic事件（无外生项）：每只股票将四类事件作为四个维度拟合。
"""
import os
import json
import numpy as np
...
from hawkes_full_vs_simple_4d_noexo import run_comparison_4d_noexo
...
def fit_stock_4d(stock_code: str, stock_data: Dict, output_dir: str) -> Dict:
    ...
    temp_file = f"temp/events_{stock_code}_4d_noexo.json"
    payload = []
    ...
    os.environ["OUT_TAG"] = f"{stock_code}_4d_noexo"

    try:
        run_comparison_4d_noexo(temp_file)
        result_file = f"results/comparison_4d_noexo_{stock_code}_4d_noexo.json"
        if os.path.exists(result_file):
            with open(result_file, "r", encoding="utf-8") as f:
                result = json.load(f)
            result["stock_code"] = stock_code
            result["event_type"] = "4d_noexo"
            ...
            if save_per_stock != "0":
                ...
                output_file = os.path.join(output_dir, f"{stock_code}_4d_noexo.json")
...
def generate_summary_report_4d_noexo(all_results: List[Dict], output_dir: str) -> None:
    report = {
        "summary": {
...
def main():
    ...
    output_dir = "results/toxic_events_4d_noexo"
    ...
    if len(all_results) > 0:
        ...
        generate_summary_report_4d_noexo(all_results, output_dir)
