...
- **基线模型**：**加性基线**：
$$\lambda_i(t) = \mu_i + \text{intraday effects} + \text{spread effect} + \text{excitation}$$
...
2. **spread 外生项的贡献显著但有限**：
$\gamma_{\text{spread}}$ 的线性系数解释清晰——spread 对强度有直接线性贡献，但 Model C 相对 Model B 的 LL 提升较小（+30），说明在加性基线下 spread 的边际贡献有限
...
**加性基线下**，$\gamma_{\text{spread},i}$ 直接表示标准化 spread 每增加 1 个标准差时基线强度的**线性增量**：

| 组 | BT $\gamma$ | BN $\gamma$ | ST $\gamma$ | SN $\gamma$ | 解读 |
|:--:|:---:|:---:|:---:|:---:|:-----|
...
**经济学含义**：
- **价差扩大→有毒订单减少**：高价股中，spread 每增加 1 std，BT/ST 基线强度线性下降约 0.002–0.003。这符合知情交易者在价差较大时降低交易意愿的理论预期（交易成本上升）
- **无毒订单受影响较小**：BN/SN 的 $\gamma_{\text{spread}} \approx 0$，流动性提供者对价差变化的敏感度较低
...
| **交易成本分析（TCA）** | Model C | $\gamma_{\text{spread}}$ 直接量化价差对强度的线性影响 |
