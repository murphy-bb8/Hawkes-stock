...
3. 引入订单簿不平衡（OBI）和对手方深度（log_opp_depth）作为外生驱动项，能否进一步提升模型拟合？

为此，我们设计了 **三种递进的模型变体**（加性基线风格），在统一的对数似然口径下进行公平对比。

### 技术创新与贡献

1. **外生项对模型拟合的显著提升**：实验表明，引入 OBI + log_opp_depth 后，Model C 相对 Model B 的对数似然提升按组为 +2,069 ~ +4,434，AIC/BIC 在 45 只股票上 100% 选择 Model C，说明订单簿微观结构变量对订单流强度有实质性驱动作用。

2. **可配置的多外生项框架**：Model C 采用 **可切换的外生变量设计**，通过 `exog_lists` 字典传入任意外生变量组合，EM 算法对每个变量独立更新 $\gamma_{v,i}$。`run_experiment_additive.py` 中通过 `EXOG_VARS` 配置当前使用的外生项；`test_exog_combinations.py` 支持 7 种组合（C1–C7）的对比实验，便于筛选最优外生变量。

3. **加性基线的闭式 M-step**：所有参数（μ, α, γ_exog）均有闭式解，无需数值优化，计算高效且参数可解释性强。

### 技术实现

- **估计方法**：**EM 算法**，所有参数（μ, α, γ_exog）均有 **闭式 M-step 解**，无需数值优化
- **基线模型**：**加性基线**：
$$\lambda_i(t) = \mu_i + \text{intraday effects} + \text{exogenous effects} + \text{excitation}$$
- **外生变量**：OBI（订单簿不平衡）、log_opp_depth（对手方深度），经 z-score 标准化、非负平移、max=1 归一化处理
...
无外生项                     γ_open / γ_mid / γ_close      γ_open/mid/close + Σ_v γ_v·x_v⁺(t)
统一加性基线口径             统一加性基线口径              统一加性基线口径
闭式 M-step 解              闭式 M-step 解               闭式 M-step 解
```
其中 Model C 外生项为 OBI、log_opp_depth 的加性组合。
...
### Model C — 时变基线 μ + 多外生项（加性）

$$\lambda_i(t) = \mu_{i,\text{period}(t)} + \sum_{v} \gamma_{v,i} \cdot x_v^+(t) + \sum_{j=1}^{4} \sum_{t_k^j < t} \alpha_{ij} \, \omega \, e^{-\omega(t - t_k^j)}$$

在 Model B 的基础上，将 **OBI**（订单簿不平衡）和 **log_opp_depth**（对手方深度）纳入加性基线。
各外生变量 $x_v^+(t)$ 经 z-score 标准化、非负平移、max=1 归一化处理。
$\gamma_{v,i}$ 直接表示第 $v$ 个外生变量对第 $i$ 维强度的线性贡献。
...
| 外生项系数 | $\gamma_{v,i}$ | OBI、log_opp_depth 的线性系数 | — | — | ✓ |
| 参数个数 | $k$ | 自由参数总数（用于 AIC/BIC） | 20 | 32 | 40 |
...
- **EM 算法闭式解**：
  - **E-step**：计算责任变量 $p_{n,\text{base}}$, $p_{n,\text{exog}_v}$, $p_{n,\text{excitation}}$
  - **M-step**：所有参数均有闭式更新公式
    ...
    - $\gamma_{v,i} = \frac{\sum_{n:u_n=i} p_{n,\text{exog}_v}}{\int_0^T x_v^+(t) dt}$ （每个外生变量独立更新）
...
| **Mid** | **0.683 ± 0.045** | **0.681 ± 0.044** | **0.680 ± 0.044** |
| **Low** | **0.759 ± 0.034** | **0.757 ± 0.034** | **0.756 ± 0.033** |
...
| 指标 | Model A ($k$=20) | Model B ($k$=32) | Model C ($k$=40) |
|:------:|:-----------------:|:-----------------:|:-----------------------:|
| **LL 单调性** | 基准 | LLₖ ≥ LLₐ ✓ (45/45) | LLᴄ ≥ LLₖ ✓ (45/45) |
| **LL 提升 (B−A)** | — | +4,416 ± 2,483（High/Mid 组） | — |
| **LL 提升 (C−B)** | — | — | +2,069 ~ +4,434（按组） |
...
- **AIC/BIC 一致选择 Model C**，说明 OBI + log_opp_depth 外生项显著改善拟合
- **Model C 相对 B 的 LL 提升显著**：High 组约 +2,069、Mid 组约 +3,457、Low 组约 +4,434，低价股受益最大

#### GOF 综合评分

| 价格组 | Model A (常数 μ) | Model B (时变 μ) | Model C (时变 μ + exog) |
|:------:|:-----------------:|:-----------------:|:-----------------------:|
| **High** | **0.609** | **0.606** | **0.607** |
| **Mid** | **0.568** | **0.565** | **0.566** |
| **Low** | **0.542** | **0.542** | **0.541** |
| 均值 | **0.573** | **0.571** | **0.572** |


**关键发现**：
1. **GOF 评分体现加性基线特点**：加性基线的 GOF 评分在 0.54–0.61 之间，反映了加性模型在高频数据拟合上的特征
2. **Model C 的 GOF 略优于 B**（High/Mid 组），OBI + log_opp_depth 对残差分布有轻微改善
...
- **Toxic 维度自激励更强**：
$A_{\text{BT,BT}} > A_{\text{BN,BN}}$，$A_{\text{ST,ST}} > A_{\text{SN,SN}}$
...
| 维度 | Model A (常数 μ, $k$=20) | Model B (时变 μ, $k$=32) | Model C (+exog, $k$=40) |
|:-----|:-----------------|:-----------------|:----------------|
| **复杂度** | 最低（ $\mu, \alpha, \omega$） | 中等（ $\mu_{\text{period}}$） | 最高（    $\gamma_{\text{OBI}}$, $\gamma_{\text{log\_opp\_depth}}$） |
| **LL 单调性** | 基准 | LLₖ ≥ LLₐ (100%) | LLᴄ ≥ LLₖ (100%) |
| **AIC 最优** | 0/45 | 0/45 | **45/45 (100%)** |
| **BIC 最优** | 0/45 | 0/45 | **45/45 (100%)** |
| **GOF 评分** | 0.573 | 0.571 | 0.572 |
| **GOF 4/4 通过** | 0% | 0% | 0% |
| **参数可解释性** | 高 | 高 | 高（ $\gamma_v$ 线性系数直观） |
| **适用场景** | 基准对比 | 日内时段分析 | **推荐默认选择** |

### 核心发现

1. **Model C（时变 μ + OBI + log_opp_depth）是最佳模型**：在统一加性基线口径下，AIC/BIC 均以 100% 的比例选择 Model C。LL 单调性 $\text{LL}_C \geq \text{LL}_B \geq \text{LL}_A$ 在所有 45 只股票上 100% 成立，符合嵌套模型的理论预期

2. **外生项（OBI、log_opp_depth）的贡献显著**：
$\gamma_{\text{OBI}}$、$\gamma_{\text{log\_opp\_depth}}$ 的线性系数解释清晰。Model C 相对 Model B 的 LL 提升按组为 +2,069 ~ +4,434，低价股受益最大，说明订单簿不平衡和对手方深度对订单流强度有实质性驱动作用

3. **价格效应是最稳健的发现**：三种模型一致显示：
   - 分枝比：High (0.61) < Mid (0.68) < Low (0.76)
   - 自激励：
   $A_{\text{BT,BT}}$: High (0.63) < Mid (0.72) < Low (0.79)
...
### 三、Model C 的 $\gamma_{\text{OBI}}$、 $\gamma_{\text{log\_opp\_depth}}$ 经济学解释

**加性基线下**，$\gamma_{v,i}$ 直接表示标准化外生变量每增加 1 单位时基线强度的**线性增量**：

**$\gamma_{\text{OBI}}$（订单簿不平衡）**：OBI 为正表示买盘深度大于卖盘。典型模式为 BT/BN 维度系数为正（买盘不平衡时买方订单增加），ST 维度接近零或为负。

**$\gamma_{\text{log\_opp\_depth}}$（对手方深度）**：对手方深度越大，流动性越好。BN/SN 维度的系数通常为正且较大，反映流动性提供者在深度充足时更活跃；Toxic 维度相对较小。

**经济学含义**：
- **OBI 驱动买卖方向**：买盘不平衡时买方订单（BT/BN）强度上升，符合订单流不平衡对价格发现的驱动作用
- **深度促进流动性**：log_opp_depth 对 BN/SN 的正向效应强于 BT/ST，说明流动性提供者对订单簿深度更敏感
- **低价股外生效应更显著**：Model C 相对 B 的 LL 提升在 Low 组最大（+4,434），低价股订单流受微观结构变量影响更强

### 四、模型选择的实践启示

| 应用场景 | 推荐模型 | 理由 |
|:---------|:---------|:-----|
| **订单流预测** | Model C | AIC/BIC 最优，捕捉 OBI、深度对订单流的影响 |
| **交易成本分析（TCA）** | Model C | $\gamma_v$ 直接量化订单簿变量对强度的线性影响 |
| **市场监控/异常检测** | Model A | 最简洁，分枝比直接反映自激励强度 |
| **做市策略优化** | Model C | 同时考虑日内时段和订单簿微观结构 |
...
4. **外生变量选择**：当前使用 OBI + log_opp_depth，未来可尝试 re_spread、订单簿其他变量或组合筛选
...
├─ test_exog_combinations.py     # 外生变量组合测试：C1–C7 对比（可选）
├─ data/                          # 数据目录
...
# 2. 外生变量组合测试（可选，高/中/低各 1 股 × C1–C7）
python test_exog_combinations.py
# 或指定组合：python test_exog_combinations.py --configs C6 C7

# 3. 查看实验结果
# 原始结果：results_additive/experiment_results.json
# 外生组合测试：results_additive/exog_combination_test.json
...
| Model C | 时变基线 + 外生项 EM | 视 Cython 而定 | 网格搜索 + EM（含 OBI、log_opp_depth 积分） |
...
| Model C | 视 Cython 而定 | 视 Cython 而定 | EM 网格搜索（含 OBI、log_opp_depth） |
...
| `gamma_exog_bar.png` | Model C 外生项（OBI、log_opp_depth）效应条形图 |
