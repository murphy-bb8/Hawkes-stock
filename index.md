...
- **EM 算法闭式解**：所有参数（μ, α, γ_exog）均有闭式 M-step 解，无需数值优化
- **加性基线模型**：$\lambda_i(t) = \mu_i + \text{(intraday)} + \text{(exogenous)} + \text{(excitation)}$
- **三种递进模型**：Model A (常数 μ) → Model B (时变 μ) → Model C (时变 μ + OBI、log_opp_depth 外生项)
...
**关键发现**：
- Model C（时变 μ + OBI + log_opp_depth）是明确胜出者
- 价格效应稳健：分枝比 High(0.61) < Mid(0.68) < Low(0.76)
- 开盘效应显著：开盘 30 分钟事件到达率为基准的 1.2–1.4 倍

## 数学公式示例

$$\lambda_i(t) = \mu_{i,\text{period}(t)} + \sum_{v} \gamma_{v,i} \cdot x_v^+(t) + \sum_{j=1}^{4} \sum_{t_k^j < t} \alpha_{ij} \, \omega \, e^{-\omega(t - t_k^j)}$$

其中 Model C 外生项为 OBI、log_opp_depth。$\mu_{i,\text{period}(t)}$ 使用分段常数哑变量刻画日内时段效应。

## 技术实现

- **基线模型**：$\lambda_i(t) = \mu_i + \text{intraday effects} + \text{exogenous effects} + \text{excitation}$
